<template>
  ...
  <div>
    <h3 class="md-display-3" v-if="independent" style="margin:15px 0;">Create Warning</h3>
    <h4 class="md-display-1" v-if="!independent" style="margin:15px 0;">Add Warning</h4>
    <form novalidate  @submit.prevent="validate">
      ...
      <md-button type="submit" class="md-raised md-primary" style="margin:0" :disabled="sending">Save Warning</md-button>
    </form>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      save: function () {
        this.sending = true
        if (this.form.id > 0) {
          triggerServices.putPredicateOtherUpdate(this.form, 'warning')
            .then((response) => {
              this.$store.dispatch('showSnackBar', 'Warning has been updated successfully.')
              this.$router.push('/triggers')
            })
        } else {
          triggerServices.postPredicateOtherCreate(this.form, 'warning')
            .then((response) => {

...
module.exports = (app) => {
  app.get(config.apiBaseUrl + '/tangible/thing', validationMiddleware.validate(), (req, res) => {
  ...
  app.get(config.apiBaseUrl + '/tangible/thing/:id', validationMiddleware.validate(), (req, res) => {
  ...
  app.get(config.apiBaseUrl + '/tangible/thing/:id/poi', validationMiddleware.validate(), (req, res) => {
    thing.find({include: [poi, author, detectable], where: {id: req.params.id}}).then((object) => {
      if (object === null) {
        res.json([])
      } else {
  ...
  app.post(config.apiBaseUrl + '/tangible/thing', validationMiddleware.validate(validationRules.tangilbe.thing), (req, res) => {
    thing.create({
      name: req.body.name,
      urn: req.body.urn,
  ...
  app.put(config.apiBaseUrl + '/tangible/thing/:id', validationMiddleware.validate(validationRules.tangilbe.thing), (req, res) => {
  ...
  app.delete(config.apiBaseUrl + '/tangible/thing/:id', validationMiddleware.validate(), (req, res) => {
    thing.find({where: {id: req.params.id}}).then((object) => {
      if (object !== null) {
